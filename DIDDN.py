...
class DIDDN(nn.Module):
    ...
    def initialize_author(self, author_id):
        if author_id not in self.states:
            self.states[author_id] = {
                ...
                'b': torch.zeros(1, 1),
                'mu_theta': torch.zeros(1, 1),
                'sigma_theta': torch.zeros(1, 1),
                'mu_b': torch.zeros(1, 1),
                'sigma_b': torch.zeros(1, 1),
    ...
    def forward(self, author_id, topic_label, input_ids, attention_mask):
        ...
        if update_z1:
            z1 = self.exponential_moving_average(z1, Z)
        if update_z2:
            z2 = self.exponential_moving_average(z2, Z)
...
def loss_function(predictions, targets, mu_theta, sigma_theta, mu_b, sigma_b,
                  ...
                  theta_current, b_current, epoch, n_epochs, lambda1, lambda2, epsilon=1e-8):
    ...
    total_loss = mse_loss + lambda1 * (kl_loss_theta + kl_loss_b) + lambda2 * (stability_loss_theta + stability_loss_b)
    return total_loss
    ...
    for epoch in range(n_epochs):
        ...
        for batch in data_loader:
            ...
            for input_id, mask, target, author_id, topic_label in zip(input_ids, attention_mask, targets, author_ids,
                                                                      topic_labels):
                y_hat, z1, z2 = model(author_id, topic_label, input_id, mask)
                ...
                loss = loss_function(y_hat, target.unsqueeze(0), state['mu_theta'], state['sigma_theta'], state['mu_b'],
                                     ...
                                     epoch, n_epochs, lambda1=0.5, lambda2=0.1)
    ...
    with torch.no_grad():
        for batch in data_loader:
            ...
            for input_id, mask, author_id, topic_label in zip(input_ids, attention_mask, author_ids, topic_labels):
                ...
                batch_z1.append(z1.cpu().numpy())
                batch_z2.append(z2.cpu().numpy())

            predictions.extend(batch_predictions)
            z1_values.extend(batch_z1)
            z2_values.extend(batch_z2)

    return predictions, z1_values, z2_values